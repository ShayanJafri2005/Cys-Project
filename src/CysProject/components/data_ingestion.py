...
class DataIngestion:
    def __init__(self,data_ingestion_config:DataIngestionConfig) -> None:
        try:
            ...
            self.engine = create_engine(f"sqlite:///{self.data_ingestion_config.database_name}")
    ...
    def export_collection_as_dataframe(self):
        """
        Export table from SQLite database as a pandas DataFrame
        """
        try:
            collection_name = self.data_ingestion_config.collection_name  # e.g., "NetworkData"

            # Use pandas to read from SQLAlchemy engine
            query = f"SELECT * FROM {collection_name}"
            dataframe = pd.read_sql(query, self.engine)

            logging.info(f"Data loaded successfully from table: {collection_name}")
            return dataframe
        except Exception as e:
            raise NetworkSecurityException(e,sys)
    ...
    def export_data_into_feature_store(self,dataframe: pd.DataFrame):
        try:
            ...
            dir_name = os.path.dirname(feature_store_dir)
    ...
    def initiate_data_ingestion(self):
        try:
            ...
            dataframe_after_split = self.export_data_into_feature_store(dataframe)
            self.split_data_as_train_test(dataframe_after_split)
            dataingestion_artifact = DataIngestionArtifact(trained_file_path=self.data_ingestion_config.train_file_path,
